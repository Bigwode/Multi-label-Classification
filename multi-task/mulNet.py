...
def build_vgg_raw(img_width, img_height):
    ...
    # x = Dropout(0.5)(x)
    predict_class = Dense(2, activation='softmax', name='predict_class')(x)
    predict_attri = Dense(8, activation='sigmoid', name='predict_attri')(x)
    model_all = Model(inputs=vgg_model.input, outputs=[predict_class, predict_attri])  # , predict_attri
